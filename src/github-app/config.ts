...
export function loadSyncConfig(): SyncConfig {
  ...
  return { trackerUrl, trackerToken, timeoutMs: Number(process.env.TRACKER_TIMEOUT_MS ?? 10_000), maxRetries: Number(process.env.TRACKER_MAX_RETRIES ?? 3), retryDelayMs: Number(process.env.TRACKER_RETRY_DELAY_MS ?? 500) };
}
